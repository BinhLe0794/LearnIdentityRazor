...
{
   public AddRoleClaimModel(RoleManager<IdentityRole> roleManager, MyBlogContext myBlogContext) : base(
      roleManager,
      myBlogContext)
   {
   }
   ...
   {
      if( string.IsNullOrEmpty(roleId) ) RedirectToPage("Index");
   ...
   public async Task<IActionResult> OnPost(string roleId)
   {
      ...
      var isAny = (await _roleManager.GetClaimsAsync(_role)).Any(x =>
         x.Type == Input.Type && x.Value == Input.Value);
      if( isAny )
      {
